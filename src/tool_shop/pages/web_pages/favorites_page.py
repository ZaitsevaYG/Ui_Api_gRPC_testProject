...
class FavoritesPage:
    def __init__(self, page: Page, product: Product):
        ...
        self.product_in_favorites = page.locator(f"[data-test='favorite-{product.id.lower()}']")
        self.product_name_in_fav = page.locator("[data-test='product-name']")
        self.product_description_in_fav = page.locator("[data-test='product-description']")
        self.product_delete_from_fav = page.locator("[data-test='delete']")
    ...
    def check_product_in_favorites_and_delete(self):
        ...
        with allure.step("Проверка возможности удаления товара из избранного"):
            ...
            expect(self.no_favorites_message.last).to_be_visible()
            expect(self.no_favorites_message.last).to_have_text(
